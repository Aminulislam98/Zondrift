...
// ── Main component
export default function EditForm({ existingData }) {
  ...
  const handleSubmit = async (e) => {
    ...
    const editedForm = Object.fromEntries(form.entries());

    // ── attach array states manually ──
    editedForm.highlights = highlights.filter((h) => h.trim() !== "");
    editedForm.included = included.filter((h) => h.trim() !== "");
    editedForm.notIncluded = notIncluded.filter((h) => h.trim() !== "");

    // ── convert numbers ──
    editedForm.price = Number(editedForm.price);
    editedForm.rating = Number(editedForm.rating);
